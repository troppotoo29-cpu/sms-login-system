...
<h2 class="present-title">Present Students</h2>
...
"https://script.google.com/macros/s/AKfycbw4Czo7dh2-x7g63n8Hv-DdwJot9r3dtiCTbpuPwUcBiX8Fy2cgoDo9pljf1hBDdEk2/exec?action=classes"
);
...
if(!Array.isArray(classes)){
  ...
  alert("Classes returned are not an array");
...
alert("LOAD CLASSES ERROR: " + error);
...
const url =
"https://script.google.com/macros/s/AKfycbw4Czo7dh2-x7g63n8Hv-DdwJot9r3dtiCTbpuPwUcBiX8Fy2cgoDo9pljf1hBDdEk2/exec?class="
...
// PRESENT
(data.presentStudents || []).forEach(s => {
...
const loginTime =
student.login
? String(student.login).match(/\d{2}:\d{2}:\d{2}/)?.[0] || ""
: "";

const logoutTime =
student.logout
? String(student.logout).match(/\d{2}:\d{2}:\d{2}/)?.[0] || ""
: "";
</tr>
`;
...
const loginTime =
s.login
? String(s.login).match(/\d{2}:\d{2}:\d{2}/)?.[0] || ""
: "";

const logoutTime =
s.logout
? String(s.logout).match(/\d{2}:\d{2}:\d{2}/)?.[0] || ""
: "";
